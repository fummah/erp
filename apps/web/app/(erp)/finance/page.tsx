...
import {
  ...
  DashboardOutlined, DollarOutlined, FallOutlined, FileTextOutlined, FundOutlined, PercentageOutlined,
...
const quickLinks = [
  ...
  { href: '/finance/reports', label: 'Financial Reports', desc: 'P&L, balance sheet & cash flow', icon: <FundOutlined />, color: '#2563eb' },
  { href: '/finance/profit-loss', label: 'Profit & Loss', desc: 'Revenue and expenses by period', icon: <RiseOutlined />, color: '#16a34a' },
  { href: '/finance/balance-sheet', label: 'Balance Sheet', desc: 'Assets, liabilities and equity', icon: <BankOutlined />, color: '#0d9488' },
  { href: '/finance/cashflow', label: 'Cash Flow', desc: 'Cash in and out over time', icon: <SwapOutlined />, color: '#7c3aed' },
...
export default function FinanceDashboard() {
  ...
  const netProfit = pnl.data?.netProfit;
  const recent = (journals.data || []).slice(0, 4);

  return (
    <div className="nex-fade">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <StatCard icon={<BankOutlined />} label="Total assets" value={fmtMoney(bs.data?.totals?.ASSET)} color="#003366" hint="Balance sheet" />
        <StatCard icon={<RiseOutlined />} label="Revenue" value={fmtMoney(pnl.data?.totals?.revenue)} color="#0ea5e9" hint="Profit & loss" />
        <StatCard icon={<FallOutlined />} label="Net profit" value={fmtMoney(netProfit)} color={(netProfit || 0) >= 0 ? '#10b981' : '#ef4444'} hint={(netProfit || 0) >= 0 ? 'Profitable period' : 'Loss-making period'} />
        <StatCard icon={<AccountBookOutlined />} label="Journal entries" value={fmtNumber((journals.data || []).length)} color="#8b5cf6" hint={`${accounts.data?.length || 0} chart accounts`} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-6">
        <Card className="nex-card xl:col-span-2" title="Recent journal activity" extra={<Link href="/finance/journals"><Button type="link" size="small">View all</Button></Link>} styles={{ body: { padding: 0 } }}>
          <Table size="small" rowKey="id" dataSource={recent} pagination={false} columns={[
            { title: 'Number', dataIndex: 'number', render: (v) => <span className="font-mono text-[12px] text-[#003366] font-semibold">{v}</span> },
        ...
        <Card className="nex-card" title="Accounts by type" styles={{ body: { padding: 0 } }}>
          <div className="divide-y divide-[#f2f3f9]">
            {Object.entries(bs.data?.totals || {}).filter(([k]) => ['ASSET', 'LIABILITY', 'EQUITY'].includes(k)).map(([k, v]) => (
              <div key={k} className="flex items-center justify-between px-5 py-3">
                <span className="text-[13px] font-medium text-[#5a6080]">{k}</span>
                <span className="text-[14px] font-bold text-[#171a2e]">{fmtMoney(v)}</span>
              </div>
            ))}
          </div>
        </Card>
